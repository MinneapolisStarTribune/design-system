...
describe('Link Accessibility', () => {
  ...
  it('has no violations with trailing icon', async () => {
    const { checkA11y } = await renderAndCheckA11y(
      <Link size="medium" href="https://example.com" icon={<ArrowRightIcon />} iconPosition="end">
        Continue
      </Link>
    );
    await checkA11y();
  ...
  it('has no violations with leading icon', async () => {
    const { checkA11y } = await renderAndCheckA11y(
      <Link size="small" href="https://example.com" icon={<ArrowRightIcon />} iconPosition="start">
        Back
      </Link>
    );
    await checkA11y();
