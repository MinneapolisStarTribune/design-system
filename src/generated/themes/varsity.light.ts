/**
 * Do not edit directly, this file was auto-generated.
 * Generated from: varsity-light tokens
 * Date: generated at build time
 */

export const colors = {
  'background-brand': '#0064CF',
  'background-brand-dark': '#003D7D',
  'background-brand-subtle': '#F0F7FF',
  'background-dark-default': '#0D0D0D',
  'background-dark-gray-01': '#27292A',
  'background-dark-gray-02': '#45474A',
  'background-light-default': '#ffffff',
  'background-light-gray-01': '#F5F6F7',
  'background-light-gray-02': '#E3E5E8',
  'background-state-attention': '#D63129',
  'base-black': '#000000',
  'base-white': '#ffffff',
  'border-brand-01': '#C8FA31',
  'border-brand-02': '#0064CF',
  'border-on-dark-strong-01': '#A7ADB4',
  'border-on-dark-strong-02': '#ffffff',
  'border-on-dark-subtle-01': '#45474A',
  'border-on-dark-subtle-02': '#585C5F',
  'border-on-light-strong-01': '#45474A',
  'border-on-light-strong-02': '#0D0D0D',
  'border-on-light-subtle-01': '#E3E5E8',
  'border-on-light-subtle-02': '#A7ADB4',
  'border-state-attention-on-dark': '#F87171',
  'border-state-attention-on-light': '#D63129',
  'border-state-disabled-on-dark': '#45474A',
  'border-state-disabled-on-light': '#C8CCD0',
  'border-state-focus': '#0D0D0D',
  'brand-01': '#C8FA31',
  'brand-02': '#0064CF',
  'brand-03': 'linear-gradient(to right, #C8FA31, #65CC5C)',
  'brand-03-from': '#C8FA31',
  'brand-03-to': '#65CC5C',
  'brand-04': 'linear-gradient(to right, #1683F8, #0050A6)',
  'brand-04-from': '#1683F8',
  'brand-04-to': '#0050A6',
  'button-brand-accent-filled-background': 'linear-gradient(to right, #C8FA31, #65CC5C)',
  'button-brand-accent-filled-hover-background': '#000000',
  'button-brand-accent-filled-hover-border': 'linear-gradient(to right, #C8FA31, #65CC5C)',
  'button-brand-accent-filled-hover-text': '#000000',
  'button-brand-accent-filled-text': '#000000',
  'button-brand-accent-ghost-background': 'rgb(0 0 0 0)',
  'button-brand-accent-ghost-hover-background': 'rgb(200 250 49 / 16%)',
  'button-brand-accent-ghost-hover-text': '#C8FA31',
  'button-brand-accent-ghost-text': '#C8FA31',
  'button-brand-accent-outlined-background': 'rgb(0 0 0 0)',
  'button-brand-accent-outlined-border': 'linear-gradient(to right, #C8FA31, #65CC5C)',
  'button-brand-accent-outlined-hover-background': 'rgb(200 250 49 / 16%)',
  'button-brand-accent-outlined-hover-border': 'linear-gradient(to right, #C8FA31, #65CC5C)',
  'button-brand-accent-outlined-hover-text': '#C8FA31',
  'button-brand-accent-outlined-text': '#C8FA31',
  'button-brand-filled-background': '#0064CF',
  'button-brand-filled-hover-background': '#003D7D',
  'button-brand-filled-hover-text': '#ffffff',
  'button-brand-filled-text': '#ffffff',
  'button-brand-ghost-background': 'rgb(0 0 0 0)',
  'button-brand-ghost-hover-background': 'rgb(0 100 207 / 8%)',
  'button-brand-ghost-hover-text': '#0064CF',
  'button-brand-ghost-text': '#0064CF',
  'button-brand-outlined-background': 'rgb(0 0 0 0)',
  'button-brand-outlined-border': '#0064CF',
  'button-brand-outlined-hover-background': 'rgb(0 100 207 / 8%)',
  'button-brand-outlined-hover-text': '#0064CF',
  'button-brand-outlined-text': '#0064CF',
  'button-neutral-filled-background': '#0D0D0D',
  'button-neutral-filled-hover-background': '#45474A',
  'button-neutral-filled-hover-text': '#ffffff',
  'button-neutral-filled-text': '#ffffff',
  'button-neutral-ghost-background': 'rgb(0, 0, 0, 0)',
  'button-neutral-ghost-hover-background': '#F5F6F7',
  'button-neutral-ghost-hover-text': '#0D0D0D',
  'button-neutral-ghost-text': '#0D0D0D',
  'button-neutral-outlined-background': 'rgb(0, 0, 0, 0)',
  'button-neutral-outlined-border': '#0D0D0D',
  'button-neutral-outlined-hover-background': '#F5F6F7',
  'button-neutral-outlined-hover-border': '#0D0D0D',
  'button-neutral-outlined-hover-text': '#0D0D0D',
  'button-neutral-outlined-text': '#0D0D0D',
  'button-neutral-utility-border': '#E3E5E8',
  'button-neutral-utility-hover-border': '#0D0D0D',
  'button-neutral-utility-hover-text': '#0D0D0D',
  'button-neutral-utility-text': '#45474A',
  'cobalt-blue-100': '#E6F2FF',
  'cobalt-blue-200': '#CCE5FF',
  'cobalt-blue-300': '#ADD5FF',
  'cobalt-blue-400': '#7BBAFF',
  'cobalt-blue-50': '#F0F7FF',
  'cobalt-blue-500': '#48A0FF',
  'cobalt-blue-600': '#1683F8',
  'cobalt-blue-700': '#0064CF',
  'cobalt-blue-800': '#0050A6',
  'cobalt-blue-900': '#003D7D',
  'cobalt-blue-950': '#002955',
  'emerald-green-100': '#D8F3E7',
  'emerald-green-200': '#AAE3CB',
  'emerald-green-300': '#91D4B7',
  'emerald-green-400': '#66C29B',
  'emerald-green-50': '#EBF9F3',
  'emerald-green-500': '#32B87F',
  'emerald-green-600': '#00AC63',
  'emerald-green-700': '#009958',
  'emerald-green-800': '#00854B',
  'emerald-green-900': '#006B3D',
  'emerald-green-950': '#004A2B',
  'forest-green-100': '#E1F0EB',
  'forest-green-200': '#C4E3D7',
  'forest-green-300': '#A6D4C3',
  'forest-green-400': '#8EC1AF',
  'forest-green-50': '#EEF6F4',
  'forest-green-500': '#75AF9B',
  'forest-green-600': '#50927B',
  'forest-green-700': '#2E725B',
  'forest-green-800': '#165A42',
  'forest-green-900': '#05442E',
  'forest-green-950': '#003320',
  'icon-brand-01': '#C8FA31',
  'icon-brand-02': '#0064CF',
  'icon-brand-03': '#003D7D',
  'icon-on-dark-primary': '#ffffff',
  'icon-on-dark-secondary': '#F9FAFA',
  'icon-on-dark-tertiary': '#E3E5E8',
  'icon-on-light-primary': '#0D0D0D',
  'icon-on-light-secondary': '#45474A',
  'icon-on-light-tertiary': '#737A82',
  'icon-state-attention-on-dark': '#F87171',
  'icon-state-attention-on-light': '#D63129',
  'icon-state-disabled-on-dark': '#737A82',
  'icon-state-disabled-on-light': '#A7ADB4',
  'lime-100': '#F1FECC',
  'lime-200': '#E5FD9D',
  'lime-300': '#DBFA7D',
  'lime-400': '#C8FA31',
  'lime-50': '#f7fee7',
  'lime-500': '#B6E32B',
  'lime-600': '#96BE23',
  'lime-700': '#88AB21',
  'lime-800': '#668019',
  'lime-900': '#445511',
  'lime-950': '#38470D',
  'navy-blue-100': '#DBEAF5',
  'navy-blue-200': '#C1D6E6',
  'navy-blue-300': '#9BBBD4',
  'navy-blue-400': '#7CA6C5',
  'navy-blue-50': '#EBF3F9',
  'navy-blue-500': '#5489B2',
  'navy-blue-600': '#3E75A2',
  'navy-blue-700': '#1B5A8B',
  'navy-blue-800': '#0D4673',
  'navy-blue-900': '#003966',
  'navy-blue-950': '#002D52',
  'neutral-100': '#F5F6F7',
  'neutral-200': '#E3E5E8',
  'neutral-300': '#C8CCD0',
  'neutral-400': '#A7ADB4',
  'neutral-50': '#F9FAFA',
  'neutral-500': '#888F96',
  'neutral-600': '#737A82',
  'neutral-700': '#585C5F',
  'neutral-800': '#45474A',
  'neutral-900': '#27292A',
  'neutral-950': '#0D0D0D',
  'orange-100': '#FFE1CC',
  'orange-200': '#FEC49A',
  'orange-300': '#FDA668',
  'orange-400': '#FB8937',
  'orange-50': '#fff7ed',
  'orange-500': '#F96A06',
  'orange-600': '#D8540D',
  'orange-700': '#B2480B',
  'orange-800': '#953404',
  'orange-900': '#7D2D12',
  'orange-950': '#5C1C0A',
  'overlay-black': 'rgb(0 0 0 / 60%)',
  'overlay-black-subtle': 'rgb(0 0 0 / 30%)',
  'overlay-white': 'rgb(255 255 255 / 60%)',
  'overlay-white-subtle': 'rgb(255 255 255 / 35%)',
  'purple-100': '#E3DBF0',
  'purple-200': '#C8B9DF',
  'purple-300': '#BAA8D7',
  'purple-400': '#9D86C6',
  'purple-50': '#F1EDF7',
  'purple-500': '#7F61B3',
  'purple-600': '#6E4BAA',
  'purple-700': '#5B3C90',
  'purple-800': '#513286',
  'purple-900': '#38225E',
  'purple-950': '#2A1947',
  'red-100': '#fee2e2',
  'red-200': '#FECACA',
  'red-300': '#fca5a5',
  'red-400': '#F87171',
  'red-50': '#fef2f2',
  'red-500': '#F04C4C',
  'red-600': '#D63129',
  'red-700': '#B11B1B',
  'red-800': '#8F1919',
  'red-900': '#700000',
  'red-950': '#5B0B0B',
  'semantic-error-background': '#fef2f2',
  'semantic-error-foreground': '#B11B1B',
  'semantic-info-background': '#ECF8FE',
  'semantic-info-foreground': '#056394',
  'semantic-success-background': '#ECFBEA',
  'semantic-success-foreground': '#2B6626',
  'semantic-warning-background': '#fff7ed',
  'semantic-warning-foreground': '#B2480B',
  'sky-blue-100': '#CFEDFC',
  'sky-blue-200': '#9DDCFB',
  'sky-blue-300': '#6FC9F6',
  'sky-blue-400': '#3FB7F3',
  'sky-blue-50': '#ECF8FE',
  'sky-blue-500': '#0FA5F0',
  'sky-blue-600': '#0685C6',
  'sky-blue-700': '#056394',
  'sky-blue-800': '#04547C',
  'sky-blue-900': '#034363',
  'sky-blue-950': '#02324A',
  'spring-green-100': '#DCF7D9',
  'spring-green-200': '#B7EFB3',
  'spring-green-300': '#9FE798',
  'spring-green-400': '#76D86E',
  'spring-green-50': '#ECFBEA',
  'spring-green-500': '#65CC5C',
  'spring-green-600': '#48B23F',
  'spring-green-700': '#3A9932',
  'spring-green-800': '#34802D',
  'spring-green-900': '#2B6626',
  'spring-green-950': '#244721',
  'text-brand-01': '#C8FA31',
  'text-brand-02': '#0064CF',
  'text-brand-03': '#003D7D',
  'text-on-dark-primary': '#ffffff',
  'text-on-dark-secondary': '#F9FAFA',
  'text-on-dark-tertiary': '#E3E5E8',
  'text-on-light-primary': '#0D0D0D',
  'text-on-light-secondary': '#45474A',
  'text-on-light-tertiary': '#737A82',
  'text-state-attention-on-dark': '#F87171',
  'text-state-attention-on-light': '#D63129',
  'text-state-disabled-on-dark': '#737A82',
  'text-state-disabled-on-light': '#A7ADB4',
  'yellow-100': '#FFF9D1',
  'yellow-200': '#FFF2A3',
  'yellow-300': '#FFEC74',
  'yellow-400': '#FAE24B',
  'yellow-50': '#fefce8',
  'yellow-500': '#facc15',
  'yellow-600': '#EAB308',
  'yellow-700': '#ca8a04',
  'yellow-800': '#a16207',
  'yellow-900': '#713f12',
  'yellow-950': '#5C3508',
} as const;

export type Colors = typeof colors;
