...
|---|---|---|---|
| **Feature Release** | `v1.12.1 → v1.13.0` | Always | Scheduled releases with new components, token updates, or API changes. Patch digit is `0`. |
| **Hotfix / Patch Release** | `v1.12.1 → v1.12.2` | Optional | Targeted fixes to a shipped version. May be absorbed into the next feature release instead. Patch digit is > `0`. |
